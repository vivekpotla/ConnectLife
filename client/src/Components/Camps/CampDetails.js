...
export const CampDetails = () => {
  ...
  const { location, description, startDate, endDate, startTime, endTime, donorsJoined, imageURL, latitude, longitude } = locationLoc.state.camps;
  ...
  return (
    <>
      ...
      {userObj && userObj.userType === 'donor' && <BookAppointment />}
    </>
  );
}
