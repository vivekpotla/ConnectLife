...
const VolunteerRegistration = () => {
    ...
    const onSubmit = async (data) => {
        try {
            ...
            await axios.post("http://localhost:5000/api/volunteer/register", formData, axiosConfig).then((res) => {
                // Assuming res.data.payload contains the user data
                console.log("then method");
                localStorage.setItem("user", JSON.stringify({ ...res.data.payload, userType: "volunteer" }));
                navigate("/");
            }).catch((error) => {
