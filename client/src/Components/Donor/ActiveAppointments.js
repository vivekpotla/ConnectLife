...
export default function ActiveAppointments() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/donor/appointments/${JSON.parse(localStorage.getItem("user"))._id}`); // Assuming your server API endpoint is '/api/donorAppointments/:donorId'
